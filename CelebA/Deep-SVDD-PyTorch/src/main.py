...
################################################################################
@click.command()
@click.argument("dataset_name", type=click.Choice(["mnist", "cifar10", "celeba"]))
@click.argument(
    "net_name",
    type=click.Choice(
        [
...
@click.option(
    "--objective",
    type=click.Choice(["one-class", "soft-boundary"]),
    default="one-class",
...
@click.option(
    "--optimizer_name",
    type=click.Choice(["adam", "amsgrad"]),
    default="adam",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Enable hybrid mode: DSVDD + tag prediction (CelebA only).",
)
@click.option(
    "--tag_loss_weight",
    type=float,
    default=1.0,
    help="Weight for tag prediction loss in hybrid mode (default: 1.0).",
)
def main(
    dataset_name,
    net_name,
    ...
    dataset = load_dataset(
        dataset_name, data_path, normal_class, hybrid_mode=hybrid_mode
    )

    # Initialize DeepSVDD model and set neural network \phi
    deep_SVDD = DeepSVDD(
        cfg.settings["objective"], cfg.settings["nu"], hybrid_mode=hybrid_mode
    )
    deep_SVDD.set_network(net_name)

    # Log hybrid mode if enabled
    if hybrid_mode:
        logger.info("Hybrid mode enabled: DSVDD + tag prediction")
        logger.info("Tag loss weight: %.2f" % tag_loss_weight)
    ...
    if dataset_name in ("mnist", "cifar10"):
        ...
        if dataset_name == "cifar10":
            X_normals = torch.tensor(
                np.transpose(
                    dataset.test_set.test_data[idx_sorted[:32], ...], (0, 3, 1, 2)
                )
            )
