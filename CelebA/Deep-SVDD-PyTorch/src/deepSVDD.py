...
from optim.hybrid_trainer import HybridTrainer
...
class DeepSVDD(object):
    ...
    def __init__(
        self, objective: str = "one-class", nu: float = 0.1, hybrid_mode: bool = False
    ):
        ...
        self.results = {
            ...
            "test_time": None,
            "test_scores": None,
            "test_tag_accuracy": None,  # Tag prediction accuracy (hybrid mode only)
        }
    ...
    def train(
        ...
        If hybrid_mode is True, uses HybridTrainer with tag prediction.
        """

        self.optimizer_name = optimizer_name

        if self.hybrid_mode:
            # Use hybrid trainer for DSVDD + tag prediction
    ...
    def test(
        ...
        # Get tag prediction accuracy if in hybrid mode
        if self.hybrid_mode:
            self.results["test_tag_accuracy"] = self.trainer.test_tag_accuracy
